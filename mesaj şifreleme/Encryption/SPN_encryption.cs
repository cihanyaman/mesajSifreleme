...
namespace Encryption
{
    class SPN_encryption
    {
        ...
        public SPN_encryption(string plain_Text, string key)
        {
            this.key = key;
            this.bin_Key = this.stringToBinary(this.key);
        ...
        public string encrypt()
        {
            ...
            {
                data = this.bin_plainText.Substring(i, 16);// 2 karakterlik metin alın (ikili sürüm)

                for (int j = 0; j < 64; j += 16) //   2 karakter anahtarı alın(ikili sürüm)
                {
